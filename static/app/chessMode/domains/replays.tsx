 ...
 *   count_dead_clicks  -> mistakes     (?)
 *   count_rage_clicks  -> inaccuracies (?!)
 *   urls               -> the room the game was played in
 *
 * Sentry's three-tier click/error ladder maps onto chess's three-tier move
 * annotation ladder, so the columns keep their relative severity ordering.
 */

import {CHESS_PROJECT_ID} from 'sentry/chessMode/fixtures';
import type {ChessRoute} from 'sentry/chessMode/registry';

/** Chess move annotation, ordered worst-first to match Sentry's error levels. */
export type MoveSeverity = 'blunder' | 'mistake' | 'inaccuracy';

export const SEVERITY_SUFFIX: Record<MoveSeverity, string> = {
  blunder: '??',
  mistake: '?',
  inaccuracy: '?!',
};
...
export type ChessGame = {
  /** 0-10, drives the list's activity sparkline */
  activity: number;
  black: string;
  ...
  blunders: number[];
  /** Seconds. Bounded by the time control so the two columns agree. */
  duration: number;
  /** ECO code, shown as the "browser version" */
  eco: string;
  id: string;
  /** Ply indices (0-based) that were inaccuracies */
  inaccuracies: number[];
  /** Ply indices (0-based) that were mistakes */
  mistakes: number[];
  /**
  ...
  opening: string;
  rated: boolean;
  /** Rating points swapped, applied to the winner and taken from the loser */
  ratingChange: number;
...
/** The annotation for a ply, if it has one. */
export function severityOf(game: ChessGame, ply: number): MoveSeverity | undefined {
  if (game.blunders.includes(ply)) {
    return 'blunder';
  }
  if (game.mistakes.includes(ply)) {
    return 'mistake';
  }
  if (game.inaccuracies.includes(ply)) {
    return 'inaccuracy';
  }
  return undefined;
}

/** Rating delta for a player, negative when they lost. */
export function ratingDelta(game: ChessGame, forWhite: boolean) {
  if (game.result === '1/2-1/2') {
    return 0;
  }
  const whiteWon = game.result === '1-0';
  return whiteWon === forWhite ? game.ratingChange : -game.ratingChange;
}

/** Standard PGN, used by the header's "Copy PGN" action. */
export function toPgn(game: ChessGame) {
  const date = new Date(Date.now() - game.startedMinutesAgo * 60_000);
  const tags = [
    ['Event', `Pawn Patrol ${game.timeControl}`],
    ['Site', `https://pawn-patrol.dev/room/${game.roomCode}`],
    ['Date', date.toISOString().slice(0, 10).replace(/-/g, '.')],
    ['White', game.white],
    ['Black', game.black],
    ['Result', game.result],
    ['WhiteElo', String(game.whiteRating)],
    ['BlackElo', String(game.blackRating)],
    ['ECO', game.eco],
    ['Opening', game.opening],
    ['Termination', game.termination],
  ];

  const sans = game.moves.split(' ').map(token => token.split('/')[0]);
  const body: string[] = [];
  for (let i = 0; i < sans.length; i += 2) {
    body.push(`${i / 2 + 1}. ${sans[i]}${sans[i + 1] ? ` ${sans[i + 1]}` : ''}`);
  }

  return `${tags.map(([k, v]) => `[${k} "${v}"]`).join('\n')}\n\n${body.join(' ')} ${game.result}\n`;
}
...
export const CHESS_GAMES: ChessGame[] = [
  ...
  },
  {
    ...
    activity: 10,
    moves:
      'e4/e2e4 e5/e7e5 Bc4/f1c4 Nc6/b8c6 Qh5/d1h5 Nf6/g8f6 Qxf7#/h5f7',
  },
  {
    ...
    activity: 10,
    moves:
      'f3/f2f3 e5/e7e5 g4/g2g4 Qh4#/d8h4',
  },
  {
    ...
    white: 'bishop.byte',
    black: 'pawn.stark',
    opening: 'Philidor Defense, Legal Trap',
    eco: 'C41',
    timeControl: 'Blitz',
    duration: 241,
    result: '1-0',
...
function toReplayRecord(game: ChessGame) {
  ...
  return {
    ...
    // The three-tier annotation ladder, worst-first like Sentry's own columns.
    count_errors: game.blunders.length,
    count_dead_clicks: game.mistakes.length,
    count_rage_clicks: game.inaccuracies.length,
    count_infos: moveCount,
    count_segments: 1,
    count_urls: 2,
    count_warnings: game.mistakes.length + game.inaccuracies.length,
...
const routes: ChessRoute[] = [
  ...
  {
    // Saved/starred Explore queries. The replay list header reads this to
    // decide whether it is showing a saved view.
