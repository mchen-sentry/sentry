...
class TestGetNumericValueFromSpan:
    @pytest.mark.parametrize(
        ("raw_value", "number_type", "expected_result"),
        [
            ...
            (0.0, float, 0.0),
            (-12.31, float, -12.31),
            # Strings we can convert
            ("1121", int, 1121),
            ("-1121", int, -1121),
            ("12.31", float, 12.31),
            ("1121", float, 1121.0),
            # Nones count as missing data rather than invalid data
            (None, int, None),
            (None, float, None),
            # Ints are fine when we want floats - they're just missing decimals
            (1121, float, 1121.0),
            (0, float, 0.0),
            # The reverse (a float when we want an int) isn't okay, since converting would lose data
            (11.21, int, Exception),
            ("11.21", int, Exception),
    ...
    def test_behaves_as_expected_when_key_found(
        self,
        raw_value: Any,
        ...
        span = create_span("do.dog.stuff", data={"dogs_are_great": raw_value})
        keys = ["dogs_are_great"]

        result = get_numeric_value_from_span(span, keys, "dog_detector", number_type)

        if expected_result is Exception:
            assert result is None
            assert mock_log_invalid_data.call_count == 1
        else:
